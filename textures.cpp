...
Textures::Textures(const std::string filename) : img_w(0),img_h(0),count(0),size(0),img(){
    int nchannels = -1, w, h;
...
uint32_t &Textures::get(const size_t i,const size_t j,const size_t idx){
    assert(i<size && j<size && idx<count);
    return img[i+idx*size+j*img_w];
}

std::vector<uint32_t> Textures::get_scaled_column(const size_t texture_id,const size_t tex_coord,
